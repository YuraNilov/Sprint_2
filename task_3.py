class PointsForPlace:
    ...
    def get_points_for_place(place):
        points = 0
        # Проверяем валидность места и рассчитываем очки
        if place > 100:
...
class PointsForMeters:
    ...
    def get_points_for_meters(meters):
        points = 0
        # Проверяем валидность расстояния и рассчитываем очки
        if meters < 0:
...
class TotalPoints(PointsForPlace, PointsForMeters):
    def get_total_points(self, meters, place):
        # Используем методы родителей для расчета очков
...
print(total_points.get_total_points(100, 10))  # 96.0